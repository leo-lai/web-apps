  ...
  <f7-page name="pay">
    ...
    <div class="l-margin">
      <f7-list form style="margin:0;">
        <f7-list-item>
          <f7-label style="width:auto;">可使用优惠券：</f7-label>
    ...
    <div style="margin: 2rem 1rem;">
      <f7-button big fill @click="pay">确定支付</f7-button>
    </div>
...
export default {
  components: {
    ...
    pay() {
      ...
      this.$f7.showIndicator()
      this.$$api.pay.create(formData).then(({data}) => {
        if(formData.coupon_type !== 'times'){
          this.$$api.wxpay(data).then(_ => {
            this.getDeviceInfo()
            this.$f7.alert('支付成功，设备启动成功')
          }).catch(_ => {
            this.$f7.alert('支付失败，设备启动失败')
          })
        }else {
          this.getDeviceInfo()
          this.$f7.alert('该设备启动成功')
        }
      }).finally(_ => {
        this.$f7.hideIndicator()
      })
