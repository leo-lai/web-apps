		...
		<el-dialog :before-close="closeDialogInfo" title="" :visible.sync="dialogInfo.visible" width="800px">
  		<div class="l-store-info">
				<div class="_hd">
					...
					<b class="l-fs-s l-margin-r">{{dialogInfo.info.shopName}}</b>
				...
				<div class="_bd">
					<table>
						<tr>
							<td>商铺类型：</td>
							<td>{{getStoreType(dialogInfo.info.type)}}</td>
						</tr>
						<tr>
							<td>商铺地址：</td>
							<td>{{getAddress()}}</td>
			...
			<span slot="footer" class="l-margin-r-m">
				<el-button :loading="dialogInfo.loading" @click="authStore(2)">拒绝认证</el-button>
		    <el-button type="primary" :loading="dialogInfo.loading" @click="authStore(1)">通过认证</el-button>
		  </span>
		</el-dialog>
...
export default {
	...
	methods: {
      ...
      return (this.$$config.baseData.storeType.filter(item => item.key === key)[0] || {}).value || ''
		...
		getList(page = 1, rows) {
			...
			this.$$api.loan.getStoreList(this.list.filter, page, rows || this.list.rows)
			.then(({data}) => {
		...
		showDialogInfo(row) { // 商铺详情
			...
			this.$$api.loan.getStoreInfo(row.id).then(({data}) => {
		...
		submitAuth(formData = {}) {
			...
			this.$$api.loan.authStore(formData).then(_ => {
	...
	mounted() {
		this.$$event.$on('loan:tab', activeName => {
			if(activeName === 'store-auth' && this.list.data.length === 0) {
				this.getList()
			}
		})
	},
